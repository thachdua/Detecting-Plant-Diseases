...
def prep_for_model(
    ...
    # Trả về đối tượng ảnh PIL để dễ dàng lưu
    if not (to_float32 or add_batch_dim or normalize):
        return im
...
# ====================================================================
# === PHẦN THỰC THI CHÍNH (ĐỂ CHẠY ĐỘC LẬP) ===
# ====================================================================

if __name__ == "__main__":
    # --- BƯỚC 1: CẤU HÌNH ---
    # Vui lòng thay đổi đường dẫn đến ảnh bạn muốn xử lý
    INPUT_IMAGE_PATH = "D:/Plant data raw/New Plant Diseases Dataset(Augmented)/images (1).jpg"
    
    # Tên thư mục để lưu ảnh đã xử lý
    OUTPUT_FOLDER_NAME = "SavingImage"
    
    # Tùy chọn tiền xử lý
    TARGET_SIZE = (224, 224)
    STRATEGY = "letterbox" # 'letterbox' hoặc 'center-crop'
    DENOISE_STRENGTH = "none" # 'none', 'median', hoặc 'sharpen'
    ...
    try:
        ...
        if not input_path.is_file():
            ...
            print("Vui lòng kiểm tra lại đường dẫn trong biến INPUT_IMAGE_PATH.")
        else:
            print(f"Đang xử lý ảnh: {input_path.name}...")
            
            # Gọi hàm tiền xử lý để nhận lại đối tượng ảnh PIL
            # Ta đặt các tùy chọn nâng cao (normalize, float32, batch_dim) là False
            # để hàm trả về ảnh có thể lưu trực tiếp.
            processed_image = prep_for_model(
                src=input_path,
                target_size=TARGET_SIZE,
                strategy=STRATEGY,
                denoise=DENOISE_STRENGTH,
                to_float32=False, # Trả về ảnh thay vì mảng số thực
                add_batch_dim=False, # Không cần chiều batch khi lưu ảnh
                normalize=None # Không chuẩn hóa
            )
            
            # Tạo tên file đầu ra
            output_filename = f"{input_path.stem}_processed.png"
            ...
            print(f"Ảnh đã được lưu tại: {output_path.resolve()}")
